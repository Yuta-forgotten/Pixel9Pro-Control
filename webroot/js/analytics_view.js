...
(() => {
  ...
  function create(callbacks) {
    ...
    model().ranges().forEach((item) => {
      const button = el('button', 'analytics-range-btn', item.label);
    ...
    const start = el('input'); start.type = 'datetime-local'; start.setAttribute('aria-label', '开始时间');
    const end = el('input'); end.type = 'datetime-local'; end.setAttribute('aria-label', '结束时间');
    const apply = el('button', 'tiny-btn tonal', '应用时段'); apply.type = 'button';
    apply.addEventListener('click', () => callbacks.onCustom(start.value, end.value));
    const startField = el('label', 'analytics-custom-field'); startField.append(el('span', 'analytics-section-desc', '开始'), start);
    const endField = el('label', 'analytics-custom-field'); endField.append(el('span', 'analytics-section-desc', '结束'), end);
    custom.append(startField, endField, apply);
    ...
    view.root = root; view.sourceGroup = source; view.rangeGroup = range; view.custom = custom; view.customStart = start; view.customEnd = end; view.stateLine = stateLine; view.hero = hero;
  ...
  function draw(view, source, stats) {
    ...
    const values = points.map((p) => Number(p.value)).filter(Number.isFinite); const min = Math.min(...values); const max = Math.max(...values); const lo = min === max ? min - 1 : min; const hi = min === max ? max + 1 : max; const span = Math.max(1, points[points.length - 1].ts - points[0].ts);
    ...
    for (let i = 0; i <= 3; i += 1) { const y = pad.top + (plotH * i) / 3; ctx.beginPath(); ctx.moveTo(pad.left, y); ctx.lineTo(width - pad.right, y); ctx.stroke(); ctx.fillText(`${(hi - ((hi - lo) * i) / 3).toFixed(source === 'thermal' ? 1 : 0)}${source === 'thermal' ? '°' : ''}`, pad.left - 5, y + 3); }
  ...
  function update(view, payload) {
    const { source, rangeId, stats, status, capture } = payload;
    ...
    view.legend.textContent = stats?.count ? `${stats.count} 个采样点 · 覆盖 ${model().formatDuration(stats.coverageSec)}${stats.gaps?.length ? ` · ${stats.gaps.length} 段缺测` : ''}` : '暂无足够采样点';
    view.moreBody.replaceChildren();
    if (isThermal) view.moreBody.append(row('数据范围', stats?.startTs && stats?.endTs ? `${new Date(stats.startTs * 1000).toLocaleString()} — ${new Date(stats.endTs * 1000).toLocaleString()}` : '—'), row('采样点', stats?.count), row('温控阈值累计', model().formatDuration(stats?.thresholdSec)));
    else view.moreBody.append(row('有效采样点', stats?.count), row('可证明放电', Number.isFinite(stats?.consumedMah) ? `${stats.consumedMah.toFixed(2)} mAh` : '—'), row('有效差分时长', model().formatDuration(stats?.activeSec)), row('数据质量', stats?.quality));
  ...
  registerFeature('analyticsView', { create, update, loading, empty, error });
})();
