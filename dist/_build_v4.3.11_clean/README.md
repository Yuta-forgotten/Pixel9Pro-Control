...
- UECap 说明按当前 payload hash/audit 重新校准：`universal = stock`、`balanced = trial_minimal_cn_combo`、`special = global special`
- WebUI 中文文案整体重构，统一为更自然的中文口径
- WebUI 前端轮询从固定四路 `setInterval` 改为单调度器
- 页面仅拉取当前 tab 相关数据；切后台或锁屏后前端轮询暂停
- 用户闲置 `45s` 或弹窗打开时自动降频，减少前台无效唤醒
- 功耗详情保留 `45s` 短时缓存，并在网络错误时自动重试一次
- UE 能力配置切换，只重启 cellular modem，不触发 airplane mode
...
- SIM2 空槽省电：副卡槽为空时关闭 `radio instance 1`，插卡后自动恢复
...
|------|--------------|-----------------|-------------|------------|------|
| 系统原生 / `universal` | stock 等价副本 | `0E37F39C` | `7213` | `+0 / -0 / ~0` | 当前 `universal` 与 stock hash 完全一致，等价于回到原厂能力表 |
...
- 仓库里旧文档提到“`balanced` 去掉 `n41+n79+n28` 3CC”，那是早期独立 `pixel_uecap_balanced_module` 的说明，不是当前控制模块里 `balanced -> trial_minimal_cn_combo` 的映射
- 当前控制模块中的 `balanced` payload hash 与 `CA_module/trial_minimal_cn_combo` 完全一致
- 当前控制模块中的 `universal` payload hash 与原厂 stock 完全一致

- WebUI 优化页提供三选一按钮组，切换后立即生效 (bind mount)
- 切换后仅执行 `cmd phone restart-modem` 重启蜂窝 modem，不再触发 `airplane mode`
- `balanced` 对应经审计的 `trial_minimal_cn_combo` payload，与 `special` 保持独立 binarypb
- 切换后 WebUI 会自动校验当前配置和目标摘要，确认一致后才提示成功
- 不再有自动策略循环，`policy` 固定为 `manual`
...
- 功耗详情默认按“当前放电会话”展示，并额外区分“今日累计 / Android batterystats 窗口”
- 功耗详情增加 45 秒短时缓存，并在网络错误时自动重试一次，避免偶发 `Failed to fetch`
- UE 能力配置切换按钮会先进入“切换中 / 校验中”，只有当前配置和目标摘要一致后才提示成功
...
如果你仍看到这个错误，先确认安装的确实是发布页资产 `pixel9pro_control_v4.3.11.zip`，而不是源码压缩包。
