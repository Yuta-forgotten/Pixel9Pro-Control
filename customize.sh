...
. "$MODPATH/scripts/ntp_config_lib.sh" || abort
...
if [ "$UECAP_EXTERNAL" -eq 1 ]; then
    ...
    rm -f "$MODPATH/system/vendor/firmware/uecapconfig/"* 2>/dev/null \
        || { ui_print "  ✗ 无法移除不适用于 komodo 的内置 UECap payload"; abort; }
    ...
    [ -f "$MODPATH/uecap_profile.sh" ] \
        || { ui_print "  ✗ external UECap runtime script unexpectedly missing"; abort; }
    ...
    rm -f "$MODPATH/system/vendor/firmware/uecapconfig/"* 2>/dev/null \
        || { ui_print "  ✗ 无法移除不兼容的 UECap payload"; abort; }
    rmdir "$MODPATH/system/vendor/firmware/uecapconfig" 2>/dev/null || true
    rmdir "$MODPATH/system/vendor/firmware" 2>/dev/null || true
    rm -f "$MODPATH/uecap_profile.sh" 2>/dev/null \
        || { ui_print "  ✗ 无法移除 UECap 运行脚本"; abort; }
    [ ! -e "$MODPATH/uecap_profile.sh" ] \
        || { ui_print "  ✗ UECap 运行脚本仍存在, 已中止安装"; abort; }
    ui_print ""
fi
...
if [ "$_is_upgrade" -eq 0 ]; then
    ...
    _UE_VALS=$(sh "$MODPATH/uecap_profile.sh" modes 2>/dev/null) \
        || { ui_print "  ✗ 无法读取 UECap mode contract"; abort; }
    _ue_default=$(sh "$MODPATH/uecap_profile.sh" default 2>/dev/null) \
        || { ui_print "  ✗ 无法读取 UECap default contract"; abort; }
    ...
    [ "$_ue_total" -gt 0 ] \
        || { ui_print "  ✗ UECap mode contract 为空"; abort; }
    [ "$_ue_default_found" -eq 1 ] \
        || { ui_print "  ✗ UECap default 不在 mode contract 中"; abort; }
    while true; do
        _i=0; _ue_cur=""
    ...
    _ntp_idx=0
    _ntp_total=$#
    [ "$_ntp_total" -gt 0 ] 2>/dev/null || abort
    while true; do
        _i=0; _ntp_cur=""
