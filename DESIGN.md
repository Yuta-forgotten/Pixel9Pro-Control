...
CPU 参数 ownership 必须分层，不能把一次写入当成永久控制：

| 节点/参数 | owner | 处理方式 |
|---|---|---|
| `foreground/cpus` | Android framework / `system_server` | 只读观察；不写、不快照、不回滚、不作为成功条件 |
| `top-app/cpus`、`response_time_ms`、`sched_util_clamp_min`、`vendor_sched` L2 | Pixel best-effort | 明确事务写入并立即权威复读；后续 Scene/PowerHAL/framework 回写只记录 drift |
| `background/cpus`、`system-background/cpus` | Pixel transaction | 随 profile 原子写入、复读和失败回滚 |
| `scaling_min/max_freq` | ThermalHAL / PowerHAL / Scene | 不作为日用调度控制面，不抢写 |

共享 contract 的 `writeback_policy=apply_verify_once_then_observe` 和 `health_interval_s=300` 是 service、reconcile、CGI 与 WebUI 的唯一口径。健康 worker 只读；它不会因系统回写而自动重放 profile。

所有有副作用的路径遵循：

`前置验证 → 主写入 → 权威复读 → desired/effective marker 提交 → 失败回滚 → 回滚复读`

周期 worker 遇到锁、外部 owner、稳定状态或 terminal failure 时必须 no-op/defer，不重放参数。自动 mutation 使用有界重试与 terminal state；独立 health 只读，不用持续抢写修复外部调度器或 ThermalHAL 已接管的状态。系统回写造成的 volatile drift 只能通过下一次明确的 profile/owner 事务恢复。
...
- `v4.5.09`：将日用 CPU profile 收敛为 `balanced=16/64/240`、`battery=16/96/320` 与 `top-app=0-6`；固化 framework-owned foreground 边界、共享自动温控阈值，并停止 health worker 对系统回写的自动 repair。
