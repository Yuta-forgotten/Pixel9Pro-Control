...
- 修复 WebUI token 配对、UGT `external` 安全底座清理、NR 息屏降级热点识别和旧配置迁移等问题。
- 优化外部调度接管后的状态口径：让出 CPU 调度时保留温控、ZRAM、NR/SIM2、UECap 与 WebUI 能力，避免与 UGT 重复写入调度节点。
...
- 共存模式仅让出 CPU 调度；温控阈值、ZRAM/VM、NR 息屏降级、SIM2 空槽管理、后台应用限制、UECap 管理等功能仍按原逻辑运行。
...
|------|---------|------|-----------|-----------|-----------|
| 性能优先 | cpu0-7 | 还原动态 boost 上限 (cap=1024)，手动专用；不参与自动策略 | 12ms | 20ms | 80ms |
...
| `pixel9pro_baseband_trial` | 本项目可选基带模块 | CarrierSettings (3210 .pb)、China MCFG (5 .mbn)、APN、VoLTE/VoNR/WFC props |
...
- 控制模块 + UGT：建议在安装向导或 WebUI 中选择“不覆盖 Uperf / external”，本模块停止写 CPU 调度节点，保留温控、ZRAM、NR/SIM2、UECap 和 WebUI
...
**UGT 协同说明**：Uperf Game Turbo 为第三方外部调度项目，建议从其官方发布渠道安装和更新。本项目不会引导安装 UGT，只会在检测到已安装的 UGT 时提供“覆盖 / 不覆盖”选择；选择 `external` 后，本模块的前台自动 CPU 调度不再生效，避免与 UGT 互相抢写 `cpuset`、`uclamp`、`sched_pixel` 或其它调度节点。
