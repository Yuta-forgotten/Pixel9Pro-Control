# Pixel 9 Pro Control Module v4.3.18
...
- Release: `v4.3.18`
- versionCode: `53`
- Asset: `pixel9pro_control_v4.3.18.zip`
...
- SIM2 空槽：默认不自动管理；只有在 WebUI 显式开启 `SIM2 自动管理` 后，才会通过 `cmd phone radio power` / `cmd phone ims` 控制 slot 1
...
| 状态 | sleep 间隔 | 探屏方式 |
|------|-----------|----------|
| 亮屏 | 15s | sysfs `card0-DSI-1/dpms`（v4.3.18 起，IPC-free） |
| 息屏首次 | 60s | 同上 |
| 息屏后续 | 600s | 同上 |
| 已降 LTE | 300s | 同上（v4.3.18 起，此前 60s） |
| 温度突发 | 5s | 同上（用户触发, 5 分钟） |

- v4.3.18 关键变更：探屏改 sysfs 直读（替换 `dumpsys display`），`_NR_DELAY` 60s→300s，`_NR_LTE_POLL` 60s→300s，消除模块自身对 Linux kernel suspend 的设计性阻碍
- 新增 `.standby_diag_state` 低噪声诊断摘要，记录 worker 当前分支、下一次唤醒时间和 profile / NR 状态
- 新增 `待机隔离模式` 显式开关，便于把"是不是 control 模块挡住 deep sleep"收敛成可执行的 A/B 测试
...
- 优化页支持 `SIM2 自动管理`、`待机隔离模式` 两个显式开关，以及 `后台 worker 摘要` 只读诊断卡片
