...
**v4.4.12 起 WebUI 提供「省电 / 均衡 / Google 默认」三档**（卡片顺序即省电→均衡→Google 默认）；性能优先降为内部基线，需要游戏级用户态调度请切到 UGT `external` 接管。

| 模式 (WebUI 顺序) | top-app | 说明 | 小核 resp | 中核 resp | 大核 resp |
|------|---------|------|-----------|-----------|-----------|
| ① 省电 | cpu0-6 | 避免 X4 常态介入，优先控温和续航 | 32ms | 96ms | 200ms |
| ② 均衡 | cpu0-7 | 低热日用底座，兼顾视频/feed 稳态和日常 burst（默认） | 16ms | 40ms | 200ms |
| ③ Google 默认 | cpu0-7 | 回写内核原厂 `response_time_ms_nom`（本机约 9/52/165，随内核自适应）；三档里最高性能、最接近无干预，不刷 UGT 时的高性能选择 | 9ms* | 52ms* | 165ms* |
| 性能优先 | cpu0-7 | 内部基线 (force/CLI)，不在 WebUI；还原动态 boost 上限 (cap=1024)；不参与自动策略 | 12ms | 20ms | 80ms |
...
4. **首次安装**：音量键交互向导，依次配置温控偏移、CPU 调度（检测到启用中的 UGT 则默认交其接管；否则五选一：不接管／均衡／省电／Google 默认／自动）、UECap 档位（仅 APatch/KSU）、NR 降级、NTP
5. **升级安装**：自动迁移已有设置（旧 performance 调度档并入均衡；default/Google默认 保留）；若旧配置缺调度接管设置，则检测到 UGT 默认交其接管、否则默认本模块管理（不打断升级）
6. 重启
