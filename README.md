...
- Release: `v4.4.7`
- versionCode: `71`
- Asset: `pixel9pro_control_v4.4.7.zip`
...
- 选择“不覆盖 Uperf/外部调度”后，本模块不再写 CPU 调度节点，WebUI 的 profile/auto/enforce 会暂停；若未检测到 Uperf，则表示本模块 CPU 调度未启用，系统或其它外部调度保持现状。
...
- L2 全自动，无需用户操作
- 外部调度接管时跳过 L2/L3 写入，避免与 Uperf 或其它外部调度模块互相覆盖
...
- 安全：启动时轮换随机 token、CSP `script-src 'self'`、写操作强制 JSON + CORS preflight
