...
- Release: `v4.3.21`
- versionCode: `56`
- Asset: `pixel9pro_control_v4.3.21.zip`
...
| 均衡手动 | cpu0-7 | 放慢 X4 常态介入 | 16ms | 40ms | 160ms |
...
- `light` / `battery` 让小核承担 steady-state 前台杂务，并让 X4 慢介入或不介入
...
- 优化页支持 `SIM2 空槽管理`、`待机隔离模式` 两个显式开关，以及 `后台 worker 摘要` 只读诊断卡片
