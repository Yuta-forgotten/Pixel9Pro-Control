...
- Release: `v4.4.18`
- versionCode: `82`
- Asset: `pixel9pro_control_v4.4.18.zip`
...
**dry-run owner arbiter**：`scripts/owner_arbiter.sh` 目前只做 Phase A 观测，每个亮屏 worker 周期读取 top-app、fas-rs `games.toml` / `.lease_game_list`、UGT/fas-rs 状态和本模块 `.cpu_sched_owner`，把建议状态写到 `/data/adb/fas_rs/.arbiter_state` 与 `.arbiter_history`。它不会修改 `.cpu_sched_owner`，不会启停 `uperf` / `fas-rs`，也不会写 CPU/kernel 节点；创建 `/data/adb/fas_rs/.arbiter_disable` 可停止 arbiter 采样并记录 `ARB_DISABLED`。本阶段只验证门禁、debounce、lease/exit-hold 判断，不实现自动 UGT/fas-rs 动态 handoff。
